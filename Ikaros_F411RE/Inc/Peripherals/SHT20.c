...
Status_code_t SHT20ReadTemperature(I2C_alternative_t I2C, int16_t* temp){
    ...
    status=I2C_Write(I2C,SHT20_SLAVE_ADDR, &temp_reg,1);
    if(Success != status){
        return status;
    }
    Delay(100000);

    status= I2C_Read(I2C,SHT20_SLAVE_ADDR,read_data_temp,2);
    if(Success!=status){
        return status;
    }
//    Delay(500000);
...
Status_code_t SHT20ReadHumedad(I2C_alternative_t I2C, uint16_t* hum){
    ...
    status = I2C_Write(I2C,SHT20_SLAVE_ADDR, &hum_reg,1);
    if(Success != status){
        return status;
    }
    Delay(100000);

    status= I2C_Read(I2C,SHT20_SLAVE_ADDR,read_data_hum,2);
    if(Success != status){
        return status;
    }
//    Delay(500000);
